...
class ExpandedBehavior:
    ...
    Evaluation prompts are generated by EvalPromptGenerator in parallel.
    """
    ...
    # LLM-generated evaluation prompts (populated by EvalPromptGenerator)
    generated_eval_prompts: List[str] = field(default_factory=list)
    ...
    def set_eval_prompts(self, prompts: List[str]) -> None:
        """Set LLM-generated evaluation prompts.

        Called by EvalPromptGenerator after generating prompts.

        Args:
            prompts: List of actual user prompts for evaluation.
        """
        self.generated_eval_prompts = prompts

    def get_evaluation_prompts(self, max_prompts: int = 50) -> List[str]:
        """Get evaluation prompts for behavior testing.

        Priority order:
        1. LLM-generated prompts (from EvalPromptGenerator) - preferred
        2. Fallback to scenario-based prompts (legacy, for compatibility)
        ...
        """
        # Primary: Use LLM-generated prompts (actual user messages)
        if self.generated_eval_prompts:
            return self.generated_eval_prompts[:max_prompts]

        # Fallback: Legacy scenario-based prompts (for backward compatibility)
        # Note: These are descriptions, not actual prompts - less effective
        prompts = []
        ...
        # Use domains with templates as last resort
